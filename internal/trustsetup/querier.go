...
// grpcQuerier implements [trustclient.TrustQuerier] via gRPC.
type grpcQuerier struct {
	client trustpb.TrustServiceClient
}

// GetToolTrust implements [trustclient.TrustQuerier].
func (q *grpcQuerier) GetToolTrust(ctx context.Context, toolURI string) (*trustclient.ToolTrustResult, error) {
...
func (q *grpcQuerier) SubmitAttestation(ctx context.Context, req *trustclient.AttestationSubmission) (*trustclient.SubmitResult, error) {
...
func (q *grpcQuerier) RetractAttestation(ctx context.Context, attestationID, attesterDID string, signature []byte) error {
...
// Compile-time check.
var _ trustclient.TrustQuerier = (*grpcQuerier)(nil)

// NewSubmitter returns a [trustclient.TrustQuerier] backed by conn. The
// returned value satisfies the pending.Submitter interface (which is a subset
// of TrustQuerier) without importing the pending package here.
func NewSubmitter(conn *grpc.ClientConn) trustclient.TrustQuerier {
	return &grpcQuerier{client: trustpb.NewTrustServiceClient(conn)}
}

// TrustClient returns a [trustclient.Client] backed by the gRPC connection.
func (s *TrustSetup) TrustClient() *trustclient.Client {
	q := &grpcQuerier{client: trustpb.NewTrustServiceClient(s.Conn)}
	return trustclient.NewClient(q)
}
